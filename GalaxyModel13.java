 ...
 * a stellar disk which is Sersic radially and sech^2 vertically and a 3D gas
 ...
 *
 */
public class GalaxyModel13 implements GalaxyModel {
	/**
	...
	private double rMax;
	/**
	 * The half light radius of the stellar disk
	 */
	private double re;
	/**
	 * The Sersic index of the stellar disk
	 */
	private double n;
	/**
	 * The bn parameter of the Sersic profile used for the stellar disk
	 */
	private double bn;
	/**
	...
	private double z0;
	/**
	 * The volume mass density of the stellar disk in the galactic midplane
	...
	/**
	 * @param n The galaxy's Sersic index
	 ...
	 */
	public GalaxyModel13(double n, double vFinal, String sechFile) {
		this.n = n;
		bn = 2*n - 1.0/3.0;
		...
		double massToLight = pow(10, -1.578 - 0.0856*mR);
		//The mass to light ratio in solar units
		double stellarMass = pow(10, -0.4*(mR - 4.61))*massToLight*Msun;
		z0 = (0.45*vMax/100000 - 0.14)*Kpc;
		effectiveDensity = 2*pow(bn, 2*n)*surfaceBrightness*massToLight/pow(
			E, bn)/Gamma.gamma(2*n + 1)/4/z0*Msun/pow(Kpc, 2);
		re = sqrt(stellarMass*pow(bn, 2*n)/PI/pow(E, bn)/Gamma.gamma(
			2*n + 1)/effectiveDensity/4/z0);
		double r0 = sqrt(pow(10, -0.4*(mR - 4.61))/5.647/PI/surfaceBrightness)*Kpc;
		//This exponential disk scale length r0 is only used to calibrate r25 which
		//is required to get the scale length of the velocity dispersion's decline.
	...
	public double[][] calculateAccelerations(double start, double end) {
		...
		double[] radius = new double[(int) ((max(
			max(rMax, rFinalS), rFinalG))/rMax*500)];
	...
	public void recordData(String pathname) {
		...
		for (int i = 0; i < radius.length; i = i + 1) {
			...
			double w = 1.7*pow(p, -0.5)*sigmaSZ/pow(2*PI*G*effectiveDensity*pow(E,
				-bn*(pow(radius[i]/re, 1/n) - 1)), 0.5);
	...
	public double getBaryonicMass() {
		double mass = PI/pow(bn, 2*n)*re*re*pow(E, bn)*effectiveDensity*4*z0*
			Gamma.gamma(2*n + 1);
		...
		for (int i = 0; i < radius.length; i = i + 1) {
			...
			if (radius[i] <= rMax) {
				double starSurfaceDensity = 4*z0*effectiveDensity*pow(E,
					-bn*(pow(radius[i]/re, 1/n) - 1));
				...
				stellarDensity = aS + bS*pow(E, radius[i]/0.1/rMax);
			}
			if (gasDensity > 0) {
	...
	public double getStellarMass() {
		return PI/pow(bn, 2*n)*re*re*pow(E, bn)*effectiveDensity*4*z0*Gamma.gamma(
			2*n + 1);
	}
	...
	public double getSFEHI() {
		...
		double rFinalG = parameters[5];
		double[] radius = new double[(int) ((max(
			max(rMax, rFinalS), rFinalG))/rMax*2000)];
		for (int i = 0; i < radius.length; i = i + 1) {
			radius[i] = ((double) i)*rMax/2000;
		}
		...
		for (int i = 0; i < radius.length; i = i + 1) {
			...
			if (r > rMax) {
				starDensity = aS + bS*pow(E, r/0.1/rMax);
				gasDensity = aG + bG*pow(E, r/0.1/rMax);
			} else {
				starDensity = 4*z0*effectiveDensity*pow(E, -bn*(pow(r/re, 1/n) - 1));
				if (i < 200) {
					gasDensity = m*r + c;
				} else {
					double v = URC(radius[i]);
	...
	private double[] getTruncations() {
		...
		double r2 = rMax;
		double r1 = rMax - 0.001*rMax;
		double starDensity2 = 4*z0*effectiveDensity*pow(E, -bn*(pow(
			r2/re, 1.0/n) - 1));
		double starDensity1 = 4*z0*effectiveDensity*pow(E, -bn*(pow(
			r1/re, 1.0/n) - 1));
		double sigmaG2 = sigma0*pow(E, -1*r2/rG);
		double sigmaG1 = sigma0*pow(E, -1*r1/rG);
		double v2 = URC(r2);
		double k2 = v2/r2*sqrt(2*(1 + r2/v2*dURCdr(r2)));
		...
		double v1 = URC(r1);
		double k1 = v1/r1*sqrt(2*(1 + r1/v1*dURCdr(r1)));
		...
		double bS = pow(E, -10)*(starDensity2 - starDensity1)/0.01;
		double aG = gasDensity2 - (gasDensity2 - gasDensity1)/0.01;
		double bG = pow(E, -10)*(gasDensity2 - gasDensity1)/0.01;
		double rFinalS = 0.1*rMax*log(abs(aS/bS));
	...
	private double[] getInnerDensities(int i, double[] radius) {
		...
		double stellarDensity1 = 4*z0*effectiveDensity*pow(E, -bn*(
			pow(radius[i]/re, 1/n) - 1));
		...
		double stellarDensity2 = 4*z0*effectiveDensity*pow(E, -bn*(
			pow(radius[i + 1]/re, 1/n) - 1));
		...
		for (int i = 0; i < radius.length; i = i + 1) {
			...
			if (radius[i] <= rMax) {
				...
				for (int j = 0; j < height.length; j = j + 1) {
					stellarDensity[i][j] = 4*effectiveDensity*pow(E, -bn*(pow(
						radius[i]/re, 1/n) - 1))*pow(E, height[j]/z0)/pow(pow(E,
						height[j]/z0) + 1, 2);
				}
				if (i > 50) {
					double sigmaG = sigma0*pow(E, -1*radius[i]/rG);
				...
				for (int j = 0; j < height.length; j = j + 1) {
					stellarDensity[i][j] = (aS + bS*pow(E, radius[i]/0.1/rMax))/z0*pow(
			...
			double starSurfaceDensity = 4*z0*effectiveDensity*pow(E, -bn*(pow(
				radius[i]/re, 1/n) - 1));
			double sigmaG = sigma0*pow(E, -1*radius[i]/rG);
			double sigmaSZ = sqrt(2*PI*G*starSurfaceDensity*z0) + sigmaSAsymptote;
			//The stellar velocity dispersion in the z direction
