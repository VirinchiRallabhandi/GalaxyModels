 ...
 * It does this by deriving the appropriate URC, a stellar disk which is exponential
 * radially and sech^2 vertically and a 3D gas disk which maintains a constant
 * Toomre stability parameter across the two disks. The gas disk flares towards higher
 * radii as per observations. The dark matter distribution is therefore deduced
 * from the URC and baryonic matter distribution via Poisson's equation.
 * This class is solely intended to model dwarf disk galaxies, unlike all previous
 * models which only probed the spirals well. Hence, it uses the Karukes and Salucci
 * URC which has been calibrated for dwarf disk galaxies.
 ...
 *
 */
public class GalaxyModel9 implements GalaxyModel {
	/**
	...
	private double vMax;
	/**
	 * The compactness parameter of the URC for dwarf disk galaxies
	 */
	private double c;
	/**
	 * A scaling constant multiplying the URC expression so that V(rMax) = vMax
	 * actually holds in the deduced rotation curve.
	 */
	private double v0;
	/**
	 ...
	 */
	public GalaxyModel9(double vMax, String sechFile) {
		this.vMax = vMax;
		...
		stellarMass = pow(10, -0.4*(mR - 4.61))*pow(10,
			-1.578 - 0.0856*mR)*Msun;
		maxDensity = stellarMass/8/PI/z0/r0/r0;
		c = pow(10, -3.64 + 0.46*log10(stellarMass/Msun))/(r0/Kpc);
		...
		c = 1;
		double maxVFound = 0;
		for (double r = 0; r < rMax; r = r + 0.01*r0) {
			double v = URC(r);
			if (v > maxVFound) {
				maxVFound = v;
			}
		...
		v0 = vMax/maxVFound;
		System.out.println(dURCdr(r0));
		System.out.println((URC(r0 + 0.0001*r0) - URC(r0))/(0.0001*r0));
	 ...
	 */
	public double getStellarMass() {return stellarMass;}
	 ...
	 * on the URC of Karukes and Stel.
	 */
	private double URC(double r) {
		if (r == 0) {
			return 0;
		}
		double vD2 = 0.5*G*stellarMass/r0*pow(r/r0, 2)*(Bessel.i0(r/r0/2)*Bessel.k0(
			r/r0/2) + Bessel.i1(r/r0/2)*Bessel.k1(r/r0/2));
		//v^2 due to the stellar disk alone
		double mVir = pow(10, (log10(r0/Kpc) + 0.94*log10(c) + 3.99)/0.38);
		//The galaxy's virial mass in solar units
		double mBar = mVir*0.07*pow(1 + pow(mVir/pow(10, 11.34), -2), 1.85)*pow(
			pow(mVir/pow(10, 11.58), -3.34) + pow(mVir/pow(10, 11.58), 0.043), -1.05);
		//The galaxy's baryonic mass in solar units
		double mGas = mBar*Msun - stellarMass;
		//The galaxy's gas mass in SI units
		double vGas2 = G*mGas/6/r0*pow(r/r0/3, 2)*(Bessel.i0(r/r0/6)*Bessel.k0(
			r/r0/6) + Bessel.i1(r/r0/6)*Bessel.k1(r/r0/6));
		double rho0 = pow(10, -18.26 - 0.51*log10(mVir) + 3.44*log10(c))*1000;
		//The central density of the dark matter halo in SI
		//The multiply by 1000 is because Karukes and Salucci arbitrarily calibrated
		//this expression in g/cm^3 rather than SI or Msun/Kpc^3.
		double rC = pow(10, (log10(rho0/1000) + 23.14 - 2.18*log10(c))/-0.97)*Kpc;
		//The core radius of the dark matter halo in SI
		double vHalo2 = 0;
		//v^2 due to the halo
		if (r > 0) {
			vHalo2 = 2*PI*G*rho0*pow(rC, 3)*(log(1 + r/rC) - atan(r/rC) + 0.5*log(
				1 + pow(r/rC, 2)))/r;
		}
		return sqrt(vHalo2 + vGas2 + vD2)*v0;
	}
	...
	private double dURCdr(double r) {
		...
		double rC = pow(10, (log10(rho0/1000) + 23.14 - 2.18*log10(c))/-0.97)*Kpc;
		//The core radius of the dark matter halo in SI
		double mDM = 2*PI*rho0*pow(rC, 3)*(log(1 + r/rC) - atan(r/rC) + 0.5*log(
			1 + pow(r/rC, 2)));
		double dmDMdr = 2*PI*rho0*pow(rC, 3)*(1/(rC + r) - 1/rC/(pow(
			r/rC, 2) + 1) + r/(rC*rC + r*r)); 
		double dv2Halodr = -G*mDM/r/r + G/r*dmDMdr;
		double x = r/2/r0;
		double maxDensityStar = stellarMass/2/PI/r0/r0;
		double dv2Stardr = 2*PI*G*maxDensityStar*r/r0*(Bessel.i0(x)*Bessel.k0(
			x) - Bessel.i1(x)*Bessel.k1(x));
		double besselI2 = 0;
		for (int i = 0; i < 20; i = i + 1) {
			...
			iFactorial = pow(iFactorial, 2)*(i + 1)*(i + 2);
			besselI2 = besselI2 + pow(x/2, 2*i + 2)/iFactorial;
		}
		dv2Stardr = dv2Stardr + PI*G*maxDensityStar*r*r/2/r0/r0*(Bessel.i1(
			x)*Bessel.k0(x) - Bessel.i0(x)*Bessel.k1(x) - 0.5*Bessel.k1(x)*(
			Bessel.i0(x) + besselI2) + 0.5*Bessel.i1(x)*(Bessel.k0(x) + Bessel.kn(
			2, x)));
		x = r/6/r0;
		double maxDensityGas = mGas/18/PI/r0/r0;
		double dv2Gasdr = 2*PI*G*maxDensityGas*r/3/r0*(Bessel.i0(x)*Bessel.k0(
			x) - Bessel.i1(x)*Bessel.k1(x));
		besselI2 = 0;
		for (int i = 0; i < 20; i = i + 1) {
			double iFactorial = 1;
		...
		dv2Gasdr = dv2Gasdr + PI*G*maxDensityGas*r*r/18/r0/r0*(Bessel.i1(
			x)*Bessel.k0(x) - Bessel.i0(x)*Bessel.k1(x) - 0.5*Bessel.k1(x)*(
			Bessel.i0(x) + besselI2) + 0.5*Bessel.i1(x)*(Bessel.k0(x) + Bessel.kn(
			2, x)));
		return 1/URC(r)/2*(dv2Halodr + dv2Gasdr + dv2Stardr)*v0*v0;
	}
